  ...
  <div class="h-[95vh] bg-gray-100 p-4 rounded-md overflow-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 flex flex-col gap-6">
        ...
        <!-- Calendar -->
        <div class="bg-white rounded-t-2xl shadow p-6 h-[80vh] overflow-auto">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold text-gray-700">{{ monthYear }}</h2>
            <div class="flex gap-2">
              <button
                @click="prevMonth"
                class="px-3 py-1 bg-gray-200 rounded-lg hover:bg-gray-300 flex items-center gap-1"
              >
      ...
      <div class="bg-white rounded-t-2xl shadow p-6 h-auto overflow-y-auto">
        <div
...
export default {
  ...
  computed: {
    ...
    todaysEvents() {
      ...
      const appointmentsToday = this.appointments
  ...
  async mounted() {
    ...
    try {
      ...
      if (response.data) {
        let allAppointments = response.data.map((a) => ({
          ...a,
        ...
        if (this.user.role === "Dentist") {
          allAppointments = allAppointments.filter(
            (a) => a.user_id === this.user.user_id
          );
        }

        // Save to store
        this.$store.appointments = allAppointments;
      }
    } catch (error) {
