    ...
    filteredPatients() {
      ...
      if (!query) {
        return this.appointments.filter(
          (a) => a.dentist?.dentist_id === this.user?.sub
        );
      }
      ...
      return this.appointments.filter((appointment) => {
        ...
        return (
          ...
          appointment.dentist?.dentist_id === this.user?.sub
        );
      });
    },
  },
  methods: {
    ...mapActions(useFetchDataStore, ["fetchAppointments"]),
    selectPatient(appointment) {
      const patient = appointment.patient;
      const dentist = appointment.dentist;

      this.form.patient_id = patient.patient_id;
      this.form.dentist_id = dentist.dentist_id;
      this.form.scheduled_date = appointment.scheduled_date;
      this.form.appointment_time = appointment.appointment_time;
      this.form.appointment_status = appointment.appointment_status;
      this.form.medical_history = patient.medical_history;
    ...
    async fetchUser() {
      try {
        ...
        if (response.data) {
          ...
          this.form.dentist_id = this.user?.dentist_id ?? this.user?.sub;
        }
      } catch (error) {
    ...
    async submitData() {
      const form = this.$refs.dentalChartForm;
      ...
      formData.append("procedure_date", new Date().toISOString().slice(0, 10));

      // ✅ Append selected_teeth and tooth_status_map
      ...
      try {
        ...
        const dentistName = `${this.user?.last_name}, ${this.user?.first_name}`;
        ...
        const toothNumbers = this.selectedTeeth.join(", ");
        const statuses = this.selectedTeeth
          .map((tooth) => this.toothStatusMap[tooth] || "N/A")
          .join(", ");
        ...
        console.table([mergedData]); // Dev/debugging
        this.$emit("merged-data", mergedData); // You can listen to this in parent
        ...
        toast.error("Failed to add dental chart.");
        console.error(err);
      }
    },
  },
