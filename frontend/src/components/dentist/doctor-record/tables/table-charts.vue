              ...
              <tbody>
                <template v-if="groupedData.length > 0">
                  ...
                  >
                    ...
                    <td class="px-4 py-2">
                      {{ formatScheduledDate(group.latestDate) }}
                    </td>
...
import editDentalChart from "../modals/edit-dental-chart.vue";
...
export default {
  ...
  computed: {
    ...
    filteredData() {
      ...
      if (!this.user || !Array.isArray(this.dentalCharts)) {
        return [];
      }
      ...
      return this.dentalCharts.filter((item) => {
        ...
        // ⚠️ Use `this.user.sub` instead of `user_id` (based on your JWT structure)
        const isSameUser = item.user_accounts?.user_id === this.user.sub;
      ...
      this.paginatedData.forEach((item) => {
        ...
        if (!patientMap.has(pid)) {
          patientMap.set(pid, {
            ...
            rows: [item],
            latestDate: item.procedure_date, // initialize latest date
          });
        } else {
          patientMap.get(pid).rows.push(item);

          // update latestDate if this row is newer
          if (
            new Date(item.procedure_date) >
            new Date(patientMap.get(pid).latestDate)
          ) {
            patientMap.get(pid).latestDate = item.procedure_date;
          }
        }
      });

      patientMap.forEach((value) => grouped.push(value));
      return grouped;
    },
  },
  methods: {
