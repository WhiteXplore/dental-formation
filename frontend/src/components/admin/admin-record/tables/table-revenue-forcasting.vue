<template>
  <div>
    ...
    >
      <div class="flex flex-col items-center gap-4 p-6 rounded-2xl bg-white shadow-xl border">
        <svg
        ...
        <div class="flex gap-2 items-center">
          ...
          <div class="inline-flex rounded-xl border border-gray-300 bg-gray-100 p-1">
            <button
              ...
              :class="['px-4 py-2 text-sm font-semibold rounded-lg transition', forecastType === 'daily' ? 'bg-blue-600 text-white shadow' : 'text-gray-600 hover:bg-white']"
            >
              Daily
            </button>
            <button
              @click="forecastType = 'monthly'"
              :class="['px-4 py-2 text-sm font-semibold rounded-lg transition', forecastType === 'monthly' ? 'bg-blue-600 text-white shadow' : 'text-gray-600 hover:bg-white']"
            >
              Monthly
            </button>
          </div>
  ...
  <kpiCards 
    title="Avg SARIMA" 
    :value="`₱${Number(forecast?.summary?.avg_sarima ?? 0)
  ...
  <p class="text-xs text-gray-400 mt-1">
    Based on historical SARIMA forecast
          ...
          <div class="p-4 rounded-xl border flex flex-col">
            <kpiCards title="Avg Hybrid" :value="`₱${Number(forecast?.summary?.avg_hybrid ?? 0)
        .toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`" 
      color="gray"/>
            <p class="text-xs text-gray-400 mt-1">SARIMA + ML hybrid forecast adjustment</p>
          </div>
          <div class="p-4 rounded-xl border flex flex-col">
            <kpiCards title="ML Adjustment" :value="forecast?.summary?.avg_difference ?? 0" color="red"/>
            <p class="text-xs text-gray-400 mt-1">Difference between SARIMA and Hybrid forecast</p>
          </div>
          <div class="p-4 rounded-xl border flex flex-col">
  <kpiCards 
  title="Max Expected Revenue" 
  :value="`₱${Number(forecast?.summary?.max_expected_revenue ?? 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`" 
  color="yellow"
/>
  <p class="text-xs text-gray-400 mt-1">
  ...
  <div class="p-4 rounded-xl border flex flex-col">
    <kpiCards 
      ...
      :value="`₱${Number(forecast?.summary?.last_month_actual ?? 0)
        .toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`" 
      color="gray"
    />
    <p class="text-xs text-gray-400 mt-1">
  ...
  <div class="p-4 rounded-xl border flex flex-col">
    <kpiCards 
      ...
      :value="`₱${Number(forecast?.summary?.next_month_forecast ?? 0)
        .toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`" 
      color="blue"
    />
    <p class="text-xs text-gray-400 mt-1">
  ...
  <div class="bg-white p-5 rounded-xl shadow border border-gray-100 flex flex-col">
    ...
    <p class="text-xs text-gray-400 mt-1">
      {{ forecast?.forecast_confidence?.reason ?? "Confidence level based on historical model accuracy." }}
    </p>
  </div>
  ...
  <div class="flex bg-gray-100 rounded-xl p-1 shadow-inner">
    ...
    >
      ...
      <div class="bg-white p-4 rounded-xl border">
        ...
        <div v-else class="h-[30vh] flex flex-col items-center justify-center text-gray-400 text-sm">
          <svg class="w-10 h-10 mb-2 text-gray-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3 3v18h18M7 13l3-3 4 4 5-6"/>
          </svg>
      ...
      <div class="bg-white p-6 rounded-2xl shadow-md border space-y-6">
        ...
        <template v-if="forecastType === 'daily'">
          <div v-if="forecast" class="bg-white p-6 rounded-2xl shadow-md border space-y-6">
            <p class="text-gray-600 text-sm leading-relaxed">
              This analysis uses a <b>{{ forecast?.model ?? "..." }}</b> model with
              <b>{{ forecast?.seasonality ?? "..." }} seasonality</b> to forecast daily clinic revenue.
            </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-2">
                ...
                <p class="text-gray-600 text-sm">Based on historical trends and weekly seasonality.</p>
                <p :class="{
                    'text-green-600 font-semibold': forecast?.summary?.avg_sarima >= 1000,
                    'text-yellow-600 font-semibold': forecast?.summary?.avg_sarima >= 500 && forecast?.summary?.avg_sarima < 1000,
                    'text-red-600 font-semibold': forecast?.summary?.avg_sarima < 500
                  }">
                  {{ classifyRevenue(forecast?.summary?.avg_sarima ?? 0) }}
                </p>
                <p class="text-gray-700 text-sm">Avg SARIMA revenue: ₱{{ format(forecast?.summary?.avg_sarima ?? 0) }}</p>
              </div>

              <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-2">
                <h4 class="font-semibold text-gray-800">Hybrid Forecast</h4>
                <p class="text-gray-600 text-sm">Adjusted using machine learning for recent trends.</p>
                <p :class="{
                    'text-green-600 font-semibold': forecast?.summary?.avg_hybrid >= 1000,
                    'text-yellow-600 font-semibold': forecast?.summary?.avg_hybrid >= 500 && forecast?.summary?.avg_hybrid < 1000,
                    'text-red-600 font-semibold': forecast?.summary?.avg_hybrid < 500
                  }">
                  {{ classifyRevenue(forecast?.summary?.avg_hybrid ?? 0) }}
                </p>
                <p class="text-gray-700 text-sm">Avg Hybrid revenue: ₱{{ format(forecast?.summary?.avg_hybrid ?? 0) }}</p>
              </div>
            </div>
            ...
            <div class="bg-blue-50 p-4 rounded-xl border-l-4 border-blue-500 space-y-2">
              <p class="text-gray-700 text-sm">
                ...
                <span v-if="forecast?.summary?.avg_difference > 0" class="text-green-600 font-semibold">Hybrid forecast higher</span>
                <span v-else-if="forecast?.summary?.avg_difference < 0" class="text-red-600 font-semibold">Hybrid forecast lower</span>
                <span v-else class="text-gray-600 font-semibold">Hybrid equals SARIMA</span>
              </p>
              <p class="text-gray-600 text-sm">
                <template v-if="forecast?.summary?.avg_difference > 0">Short-term revenue increase expected.</template>
                <template v-else-if="forecast?.summary?.avg_difference < 0">Short-term revenue slowdown expected.</template>
                <template v-else>Revenue expected to remain stable.</template>
              </p>

              <ul v-if="forecast?.summary?.avg_difference !== 0" class="list-disc list-inside text-gray-700 space-y-1 ml-2">
              ...
              <p class="text-gray-800 text-sm font-medium mt-2">
                ...
                <span class="text-blue-600 font-semibold">₱{{ format(forecast?.summary?.max_expected_revenue ?? 0) }}</span>
              </p>
            </div>
          </div>
      ...
      <div class="bg-white p-4 rounded-xl shadow overflow-x-auto">
        <h3 class="font-semibold mb-3">
          {{ forecastType === "daily" ? "7-Day Revenue Forecast" : "Monthly Revenue Forecast" }}
        </h3>
        ...
        <table v-else class="w-full text-sm border">
          ...
          <tbody>
            <tr>
              <td class="p-2 border">{{ formatMonth(forecast?.forecast[0]?.month) }}</td>
              <td class="p-2 border">₱{{ format(forecast?.summary.last_month_actual) }}</td>
              <td class="p-2 border font-semibold">₱{{ format(forecast?.summary.next_month_forecast) }}</td>
              <td class="p-2 border font-semibold" :class="forecast?.summary.difference >= 0 ? 'text-green-600' : 'text-red-600'">
    ...
    forecastType: {
      ...
      async handler(type) {
        ...
        try {
          ...
          this.forecast = null;
        } finally { this.loadingForecast = false; }
      },
    },
  },
  methods: {
...
renderHistoricalHybridChart() {
  ...
  const labels = [...historical.map(d => this.formatDate(d.date)), ...forecast.map(d => this.formatDate(d.date))];

  // Historical revenue: values for historical days, null for forecast days
  const historicalData = [...historical.map(d => d["Clinic Share"]), ...Array(forecast.length).fill(null)];

  // Hybrid forecast: null for historical, forecast values for forecast days
  const hybridData = [...Array(historical.length).fill(null), ...forecast.map(d => d.hybrid_forecast)];
...
,
    ...
    renderSarimaHybridChart() {
      const ctx = this.$refs.revenueChart; if (!ctx) return;
      ...
      this.chart = new Chart(ctx, {
        ...
        data: {
          labels,
          datasets: [
            { label: "Hybrid Forecast", data: hybridData, borderColor: "#2563EB", borderDash: [3,3], borderWidth: 3, tension: 0.3, fill: false },
            { label: "SARIMA Forecast", data: sarimaData, borderColor: "#FACC15", borderDash: [6,6], borderWidth: 3, tension: 0.3, fill: false },
          ]
        },
        options: this.baseChartOptions(),
      });
    ...
    renderMonthlyChart() {
      const ctx = this.$refs.revenueChart; if (!ctx || !this.forecast) return;

      const historical = this.forecast.historical || [];
      const forecast = this.forecast.forecast || [];

      const labels = [...historical.map(h => this.formatMonth(h.month)), ...forecast.map(f => this.formatMonth(f.month))];
      const historicalData = historical.map(h => h.clinic_share);
      const forecastData = [...Array(historical.length-1).fill(null), historical[historical.length-1]?.clinic_share, ...forecast.map(f => f.forecast)];
      ...
      this.chart = new Chart(ctx, {
        ...
        data: {
          labels,
          datasets: [
            { label: "Historical Revenue", data: historicalData, borderColor: "#6B7280", borderWidth: 3, tension: 0.35, pointRadius: 4 },
            { label: "Next Month Forecast", data: forecastData, borderColor: "#FACC15", borderDash: [7,7], borderWidth: 4, tension: 0.35, pointRadius: ctx => ctx.dataIndex === forecastData.length-1 ? 8 : 0, pointBackgroundColor: "#FACC15", pointBorderColor: "#F59E0B", pointBorderWidth: 3 }
          ]
        },
        options: this.baseChartOptions(),
      });
    ...
    baseChartOptions() {
      return {
        ...
        plugins: {
          ...
          tooltip: { callbacks: { label(context) { return `₱${Number(context.raw).toLocaleString()}`; } } }
        },
        scales: { y: { ticks: { callback(value) { return `₱${Number(value).toLocaleString()}`; } } } }
      };
    },
       // ------------------ XLSX Generation ------------------
    async generateXlsx() {
